...
func (*UserOpServer) AddUserFav(ctx context.Context, req *proto.UserFavRequest) (*emptypb.Empty, error) {
	...
	if result := global.DB.Where("goods=? and user=?", req.GoodsId, req.UserId).Model(&model.UserFav{}).First(&userFav); result.RowsAffected != 0 {
		return nil, status.Errorf(codes.NotFound, "收藏已存在")
	}
